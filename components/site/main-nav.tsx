...
export function MainNav() {
  ...
  return (
    <nav aria-label="Navegación principal" className="w-full border-y border-ink-800 bg-ink-950/80 backdrop-blur">
        ...
        {LINKS.map((l, i) => {
          ...
          return (
            <li key={l.href} className="flex items-center">
              {i > 0 && <span aria-hidden className="mx-3 h-3 w-px bg-ink-800 sm:mx-5" />}
              <Link
                href={l.href}
                ...
                className={cn(
                  ...
                  active ? "text-gold-400 after:scale-x-100" : "text-cream-200 hover:text-cream-50"
                )}
              >
                {l.label}
              </Link>
